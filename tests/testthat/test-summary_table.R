test_that("`pop.var` missing", {
  expect_error( summary_table( d = d_example_na %>%
                                 select( -v_pop ),
                               metric = c( "count", "rate" ),
                               var1 = "v1",
                               var2 = "v2",
...
test_that( "Ensure `summary.col.name` column total equals number of rows in input dataset.", {
  ...
  expect_equal( nrow( d_example ), count_tot_c )
...
test_that( "Ensure `summary.row.name` row total equals number of rows in input dataset.", {
  ...
  expect_equal( nrow( d_example ), count.tot.r )
  
})

test_that( "Two metrics, max", {
  
  expect_error( summary_table( d = d_example,
...
test_that( "ensure that 0's are not suppressed_", {
...
test_that( "ensure no error when rate is called but not count.", {
  ...
  expect_no_error( summary_table( d = d_example,
...
test_that( "check equality of contents of table (percentages, counts) when `percentages.rel` = 'var2'.", {
  ...
  # call function
  func_1 <- summary_table( d = d_example,
                           metric = c( "count", "percent" ),
                           var1 = "v1",
                           var2 = "v2",
                           add.summary.row = TRUE,
  ...
  ths <- names( func_1$frame )[ str_which( names( func_1$frame ), "percent" ) ] %>%
    str_remove( ., ",percent" ) %>%
    .[ .!= "All"]
  
  # use base functions to generate same table of counts
  base_1 <- table( d_example$v1, d_example$v2) %>%
    as.matrix()
  
  
  # use base functions to generate same table of percentages
  base_perc <- base_1
  for( i in 1:ncol( base_1 ) ){
    
    base_perc[, i ] <- ( 100 * base_1[, i ] / sum( base_1[, i ] ) ) %>%
...
test_that( "`var1` vector check.", {
  ...
  # call function
  call_it <- summary_table( d = d_example,
                            metric = c( "count", "percent" ),
                            var1 = c( "v1", "v3" ),
                            var2 = "v2",
                            add.summary.row = TRUE,
  ...
  var_tab <- call_it$frame$var1 %>%
    .[ .!= "Summary Row" ] 
  
  expect_true( all( var_tab %in% c( levels( as.factor( d_example[["v1"]] ) ),
                                    levels( as.factor( d_example[["v3"]] ) ) ) ) )
} )
...
test_that( "`var1` vector check when specifying `order.rows.", {
  ...
  expect_true( all( var_tab %in% c( "v1", "v3",levels( as.factor( d_example[["v1"]] ) ),
                                    levels( as.factor( d_example[["v3"]] ) ) ) ) )
  ...
  expect_true( all( var_tab %in% c( "var1", "var3",levels( as.factor( d_example[["v1"]] ) ),
                                    levels( as.factor( d_example[["v3"]] ) ) ) ) )
...
test_that( "`order.rows` check when length( var1 ) > 1", {
  ...
  expect_equal( var_tab, c( "v1", "Geo 2", "Geo 3", "Geo 1", "v3", "Other Char 2", "Other Char 3" ) )
  ...
  expect_equal( var_tab, c( "Geo 2", "Geo 3", "Geo 1", "Other Char 2", "Other Char 3" ) )
  ...
  expect_equal( var_tab, c( "this is var 1", "Geo 2", "Geo 3", "Geo 1", "this is var 3", "Other Char 2", "Other Char 3" ) )
...
test_that( "no errors with proper usage of `row.variable.labels`", {
  ...
  suppressWarnings( 
  expect_warning( summary_table( d = d_example,
                                 metric = c( "count", "percent" ),
                                 var1 = c( "v1" ),
                                 order.rows = list( v1 = c( "Geo 2", "Geo 3", "Geo 1" ),
                                                    v3= c( "Other Char 2", "Other Char 3" )),
...
expect_true( all( var_tab %in% c( "var1", "var3","Geo 2", "Geo 3", "Other Char 2" ) ) )
}
...
test_that( "ensure row labels are being ordered correctly when variables in rows have the same levels.", {
  ...
  run.samelevs <- summary_table( d = d_example,
                                 metric = c( "count", "percent" ),
                                 var1 = c( "v4", "v5" ),
                                 var2 = "v2",
                                 add.summary.row = TRUE,
  ...
  raw.count.v4 <- table( d_example$v4, d_example$v2 )
  ...
  raw.count.v5 <- table( d_example$v5, d_example$v2 )
...
run.difflevs <- summary_table( d = d_example,
                               metric = c( "count", "percent" ),
                               var1 = c( "v1", "v5" ),
...
raw.count.v1 <- table( d_example$v1, d_example$v2 )
...
test_that( "checks on order of spanning headers when multiple variables are specified in `var2`.", {
  ...
  run.mult.cols <- summary_table( d = d_example,
                                 metric = c( "count", "percent" ),
                                 var1 = c( "v1", "v5" ),
                                 order.rows = list( v1 = c( "Summary", "Geo 2", "Geo 3", "Geo 1" ),
                                                    v5 = c( "Y", "N", "" )),
  ...
  run.mult.cols.v2 <- summary_table( d = d_example,
                                  metric = c( "count", "percent" ),
                                  var1 = c( "v1", "v5" ),
                                  order.rows = list( v1 = c( "Summary", "Geo 2", "Geo 3", "Geo 1" ),
                                                     v5 = c( "Y", "N", "" )),
...
run.mult.cols.v3 <- suppressWarnings( summary_table( d = d_example,
                                   metric = c( "count", "percent" ),
                                   var1 = c( "v1", "v5" ),
                                   order.rows = list( v1 = c( "Summary", "Geo 2", "Geo 3", "Geo 1" ),
                                                      v5 = c( "Y", "N", "" )),
...
span.head_2.a <- summary_table( d = d_example,
               metric = c( "count", "percent" ),
               var1 = c( "v1", "v5" ),
               order.rows = list( v1 = c( "Summary", "Geo 2", "Geo 3", "Geo 1" ),
                                  v5 = c( "Y", "N", "" )),
...
expect_true( all( str_detect( span.head_2.a$flextable$header$dataset[ 1, 4:9 ], "var2" ) ) )

expect_true( all( str_detect( span.head_2.a$flextable$header$dataset[ 1, 10:15 ], "var3" ) ) )

# change `summary.col.pos` to "end"
span.head_2.b <- summary_table( d = d_example,
                                metric = c( "count", "percent" ),
                                var1 = c( "v1", "v5" ),
                                order.rows = list( v1 = c( "Summary", "Geo 2", "Geo 3", "Geo 1" ),
                                                   v5 = c( "Y", "N", "" )),
...
expect_true( all( str_detect( names( span.head_2.b$flextable$header$dataset[ 1, 14:15 ] ), "All\\sCol" ) ) )

expect_true( all( str_detect( span.head_2.b$flextable$header$dataset[ 1, 8:13 ], "var3" ) ) )

expect_true( all( str_detect( span.head_2.b$flextable$header$dataset[ 1, 2:7 ], "var2" ) ) )


# change `col.variable.labels` to "none"
span.head_2.c <- summary_table( d = d_example,
                                metric = c( "count", "percent" ),
                                var1 = c( "v1", "v5" ),
                                order.rows = list( v1 = c( "Summary", "Geo 2", "Geo 3", "Geo 1" ),
                                                   v5 = c( "Y", "N", "" )),
...
expect_true( all( str_detect( span.head_2.c$flextable$header$dataset[ 1, 14:15 ], "All\\sCol" ) ) )

expect_true( all( str_detect( span.head_2.c$flextable$header$dataset[ 1, 8:13 ], "Other Char\\s" ) ) )

expect_true( all( str_detect( span.head_2.c$flextable$header$dataset[ 1, 2:7 ], "^Char\\s" ) ) )


# change order of entries in col.variable.labels
span.head_2.d <- summary_table( d = d_example,
                                metric = c( "count", "percent" ),
                                var1 = c( "v1", "v5" ),
                                order.rows = list( v1 = c( "Summary", "Geo 2", "Geo 3", "Geo 1" ),
                                                   v5 = c( "Y", "N", "" )),
...
expect_true( all( str_detect( names( span.head_2.d$flextable$header$dataset[ 1, 14:15 ] ), "All\\sCol" ) ) )

expect_true( all( str_detect( span.head_2.d$flextable$header$dataset[ 1, 8:13 ], "var3" ) ) )

expect_true( all( str_detect( span.head_2.d$flextable$header$dataset[ 1, 2:7 ], "var2" ) ) )
...
test_that( "spanning header suppression when ", {
  ...
  d_front <- summary_table( d = d_example,
                        metric = c( "count", "percent" ),
                        var1 = c( "v1", "v5" ),
                        order.rows = list( v1 = c( "Summary", "Geo 2", "Geo 3", "Geo 1" ),
                                           v5 = c( "Y", "N", "" )),
  ...
  expect_true( all.equal( d_1, d_front ) )
  
  expect_true( all.equal( d_0, d_front ) )
...
test_that( "test that var1 length 1 and var2 length > 1 produces desired result", {
  ...
  short.var1 <- summary_table( d = d_example,
                          metric = c( "count", "percent" ),
                          var1 = c( "v1"),
 ...
 expect_true( short.var1$frame %>%
    filter( var1 == "Geo 2" ) %>% 
    pull( `Char 2,count`) == table( d_example$v1, d_example$v2 )[ "Geo 2", "Char 2" ] )
 
 expect_true( short.var1$frame[4, 14 ]  == table( d_example$v1, d_example$v3 )[ "Geo 1", "Other Char 3" ] )
...
expect_no_error( suppressWarnings( summary_table( d = d_example,
               metric = c( "count", "percent" ),
               var1 = c("v1", "v2" ),
               var2 = c("v4", "v5", "v3"),
...
d_out <- suppressWarnings( summary_table( d = d_example,
                        metric = c( "count", "percent" ),
                        var1 = c("v1", "v2" ),
                        var2 = c("v4", "v5", "v3"),
                        nm.var1 = "characteristic",
...
expect_true( all( d_out$flextable$header$col_keys[4:9] == c( "Y,count.x", 
                                                             "Y,percent.x", 
                                                             "N,count", 
                                                             "N,percent", 
                                                             "Y,count.y", 
...
expect_true( all( d_out$flextable$header$dataset[ 1, ] == c( "", "", "", "variable 4", 
                                           "variable 4", 
                                           "variable 5", 
                                           "variable 5", 
                                           "variable 5", 
...
d_out2 <- suppressWarnings( summary_table( d = d_example,
                         metric = c( "count", "percent" ),
                         var1 = c("v1", "v2" ),
                         var2 = c("v4", "v5", "v3"),
                         nm.var1 = "variable 1",
...
paste0( d_out2$flextable$header$dataset[ 1, ], collapse = '", "')

# check top level spanning header
expect_true( all( d_out2$flextable$header$dataset[ 1, ] == c( "", "", "", "variable 4", "variable 4", 
                                                              "variable 5", "variable 5", "v3", "v3", 
                                                              "v3", "v3", "v3", "v3") ) )

# check second level spanning header
expect_true( all( d_out2$flextable$header$dataset[ 2, ] == c( "", "Total", "Total", "Y", "Y", 
                                                              "Y", "Y", "Other Char 2", 
