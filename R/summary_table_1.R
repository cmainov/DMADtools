summary_table_1 <- function( d, var1, var2 = NULL, table.grouping = NULL, pop.var = NULL,
  ...
  return( list( flextable = t.out,
                frame = d.7,
                env = list( var.logic.2 = var.logic.2,
                            var.logic.1 = var.logic.1, 
                            tb.grp.logic = tb.grp.logic,
                            tb.grp.var2.logic = tb.grp.var2.logic,
                            order.rows.logic = order.rows.logic,
                            tb.grp.var2.wide.logic = tb.grp.var2.wide.logic,
                            new.nms = new.nms, sp.h = sp.h, h.cats = h.cats,
                            col.w = col.w, col.w.cats = col.w.cats, sum.row.nm = sum.row.nm,
                            sum.col.nm = sum.col.nm, pos.vec = pos.vec ) ) )
